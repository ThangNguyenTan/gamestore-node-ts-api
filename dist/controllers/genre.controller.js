...
class GenreController {
    async create(req, res) {
        const record = await services_1.createGenre({ ...req.body });
        return res.status(http_status_codes_1.StatusCodes.CREATED).json(record);
    }
    async getAll(req, res) {
        const records = await services_1.getAllGenres();
        return res.status(http_status_codes_1.StatusCodes.OK).json(records);
    }
    async readPagination(req, res) {
        const limit = req.query.limit || 10;
        const offset = req.query.offset;
        const records = await services_1.findGenres({ where: {}, limit, offset });
        return res.status(http_status_codes_1.StatusCodes.OK).json(records);
    }
    async readByID(req, res, next) {
        const { id } = req.params;
        const record = await services_1.getGenre(parseInt(id, 10));
        if (!record) {
    ...
    async update(req, res, next) {
        ...
        const updatedRecord = await services_1.updateGenre(parseInt(id, 10), { ...req.body });
    ...
    async delete(req, res, next) {
        ...
        const deletedRecord = await services_1.deleteGenre(parseInt(id, 10));
